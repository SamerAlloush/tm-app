...
import Icon from 'react-native-vector-icons/MaterialIcons';
import useComposeMessageLogic from './ComposeMessageLogic';
...
const ComposeMessageModal = ({ visible, onClose }) => {
  const {
    ...
    setSearchQuery,
    messageInputRef
  } = useComposeMessageLogic({ visible, onClose });
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          >
            <Text style={[
              styles.sendButton,
              (recipients.length === 0 || !message.trim()) && styles.sendButtonDisabled
            ]}>
              Send
            </Text>
          </TouchableOpacity>
